...
<div class="p-4 w-1/2 mx-auto bg-gray-100 rounded-xl shadow-xl">
    <form method="POST" action="{{route('articles.update', $article)}}" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="nombre" name="nombre" value="{{@old('nombre', $article->nombre)}}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            <x-errores.error for="nombre" />
        </div>

        <div class="mb-4">
            <label for="descripcion" class="block text-sm font-medium text-gray-700">Descripción</label>
            <textarea id="descripcion" name="descripcion" rows="4" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{@old('descripcion', $article->descripcion)}}</textarea>
        ...
        <div class="mb-4 flex items-center space-x-4">
            ...
            <div class="flex items-center">
                <input type="radio" @checked(@old('disponible', $article->disponible)=='SI' )
            ...
            <div class="flex items-center">
                <input type="radio" @checked(@old('disponible', $article->disponible)=='NO' )
        ...
        <div class="my-2">
            ...
            <div class="flex justify-between">
                ...
                <div class="ml-2">
                    <img id="preview" src="{{Storage::url($article->imagen)}}" class="w-56 aspect-video object-fill border-2 border-black rounded-md"
                        </div>
                </div>
            </div>
            ...
            <div class="flex flex-row-reverse">
                <button type="submit" class="p-2 rounded-xl font-bold text-white bg-purple-500 hover:bg-purple-700 ml-2">
